...
same data is also exposed as an MCP server (12 lean read-only tools covering 78
...
Mobilithek/DELFI, VBB (Verkehrsverbund Berlin-Brandenburg, CC-BY 4.0), GovData,
OpenStreetMap, Bundesnetzagentur, KBA and more.
...
Field names are snake_case and English across all data types, and the same
concept always uses the same name: `post_code`, `street`, `house_number`,
`place`, `name`, `start`, `end`, `distance_km`, `power_kw`, `lat`, `lon`.
`post_code` is always a five-character string (leading zeros survive),
timestamps are ISO 8601 with a time zone, and a value the source does not
provide is `null`, never an empty string. Some responses still carry older
duplicate names (`plz`, `zip`, `strasse`, `hausnummer`, `ort`, `city`,
`bezeichnung`, `beginn`, `ende`, `art`, `dist_km`, `leistung_kw`,
`einheit_typ`, plus the camelCase raw fields of the Autobahn traffic messages)
with identical values; those are deprecated, so prefer the canonical names.

> Tip: call `/api/v1/cities` first to discover the canonical city slugs (e.g.
> `koeln`, `berlin`, `hamburg`), then call any city-scoped endpoint.
>
> The `{slug}` is resolved leniently, so you rarely need the exact ASCII form:
> the German name with or without umlauts, any casing, common English exonyms and
> short forms all resolve to the canonical slug (`München`/`münchen`/`munich`/
> `munchen` → `muenchen`, `cologne` → `koeln`, `frankfurt` → `frankfurt-am-main`).
> An unknown name returns `404` with a `Meintest du ...?` hint naming the closest
> slug.
...
<!-- Endpoint count comes from docs/openapi.yaml (one operationId per operation).
     Keep it in sync: docs-site/scripts/check-endpoint-count.mjs verifies this. -->
## Data (84 cities, 124 endpoints)
...
type (its `resource` enum lists all 81 keys).
...
| **Mobility** | `transit`, live stop departures (`transit_departures` tool), `stations` (catalog), station boards by EVA (`station_board_departures`/`station_board_arrivals` tools, incl. local trains + disruptions), `station-departures`, `station-arrivals`, `traffic`, `road-events`, `webcams`, `charging`, `parking` (live occupancy), `parking-onstreet`, `park-and-ride`, `mobility-points`, `bike-parking`, `sharing`, `fuel-prices`, `bike-counts` |
| **City & people** | `base`, `geo`, `demographics`, `indicators`, `sustainability` (SDG indicators per municipality as a time series 2006-2023, Wegweiser Kommune / Bertelsmann Stiftung, CC0), `unemployment`, `tourism`, `construction`, `accidents`, `crime-stats`, `health`, `icu-live`, `holidays`, `election`, `events`, `council-papers` (municipal council information via OParl: papers, motions and resolutions per city), `pois`, playgrounds/markets/toilets and more OSM types |
...
### Pagination & channel defaults

Data-type lists (`charging`, `energy`, `events`, `transit`, OSM feature endpoints)
have a **channel-dependent default** for the same URL:

- **Direct REST** returns the **full** list in one call (`limit=null`, `returned == total`, `truncated=false`).
- **GPT Actions** (OpenAI header) and **MCP** are **bound** to a default page size, so responses stay small for agents.
- **`limit=all`** (or `?all=1`) forces the full list on any channel; `limit` + `offset` page explicitly.
- **`meta.pagination`** (`total/returned/limit/offset/truncated`) is set on every channel, so the slice is always transparent.

For `traffic`, the raw polyline is opt-in: add `include=geometry` (or `?full=1`); the default response is slim.

### Stability, changelog & roadmap

Building on InfraNode in production? Stay ahead of changes:

- **[Changelog](https://infranode.dev/en/changelog/)** lists every user-visible change (new data types, new cities, behaviour changes, fixes, deprecations), newest first. Subscribe via [RSS](https://infranode.dev/en/changelog/feed.xml).
- **[Roadmap](https://infranode.dev/en/roadmap/)** shows what is planned plus the stability policy: the API evolves additively, the envelope is stable, and changes affecting existing responses are announced in advance (typically 30+ days).
- **[Status page](https://status.infranode.dev)** and GitHub releases cover availability and versioned changes.

## Use it as an MCP server

The same API is exposed as a remote MCP server, so AI agents can call all 78
...
**Ready-made GPT:** [InfraNode: German City Data - Weather & Transit](https://chatgpt.com/g/g-6a48bf065e648191b062bc86256c1897-infranode-german-city-data-weather-transit)
...
InfraNode covers **84 German cities and 82 data types** behind one keyless, hosted
