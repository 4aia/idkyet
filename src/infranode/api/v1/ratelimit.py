...
def ANON_LIMIT() -> str:
    ...
    (z.B. Test-Override limitAnon), gilt allein dieses.

    Frisch instanziiert statt get_settings()-Cache (Konvention Toggle-Lookup), da
    @limiter.limit das Limit pro Request liest und per-Test gesetzte Env-Vars
    greifen müssen.
    """
    s = Settings()
    limits = [s.limit_anon]
...
def build_limiter(settings: Settings) -> Limiter:
    ...
    ratelimitAllowlist (z.B. Anthropic-Egress: viele Endnutzer
