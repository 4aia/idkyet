"""Free-standing async tool functions of the Atlas MCP server (DX-05).
...
Die Discovery uebernimmt ``get_city_overview``/``atlas://catalog``: beide
...
async def get_city_overview(slug: _Slug) -> ToolEnvelope:
    ...
    can pivot. Atlas keeps adding data and cities, so the catalog grows over
    time. Read-only.
    """
...
async def sources() -> ToolEnvelope:
    ...
    Takes no arguments. Shows which upstream sources Atlas bundles and
    whether each is currently active. Read-only.
    """
    # limit explizit auf MAX_LIMIT (API-Default 50 schnitt bei 76 Quellen ab:
    # eround_charging & Co. fehlten im Tool UND in atlas://sources).
