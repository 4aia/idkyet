#!/usr/bin/env bash
# deploy/cloudflare_protect.sh - Cloudflare-Schutzschicht fuer Atlas (Security-Haertung 2026-06-21)
#
...
#      das Origin.
#
# BEWUSST NICHT aktiviert: Bot Fight Mode / "Block AI bots". Atlas ist eine
...
#   - CF_ZONE_ID:  Zone-ID von woof.systems (Cloudflare Dashboard -> Overview).
#   - curl, jq.
#
# Nutzung:
...
: "${CF_ZONE_ID:?CF_ZONE_ID fehlt (Zone-ID von woof.systems)}"
...
# Diese Bots sind kommerzielle SEO-Crawler, die Atlas-Daten ohne Nutzen
...
RL_BODY="$(jq -n \
  ...
  rules: [{
    action: "block",
    description: "Atlas: harte IP-Volumengrenze (Flood-Schutz; feines Limit macht die App; verifizierte Crawler ausgenommen)",
    expression: "(http.host eq \"mcp.woof.systems\") and (not cf.client.bot)",
    ratelimit: {
...
# Greift NUR auf den Atlas-Hosts. Leeres BAD_BOTS -> Regelsatz leer (Phase aus).
WAF_EXPR=""
if [[ -n "$BAD_BOTS" ]]; then
  ...
  WAF_EXPR="(http.host eq \"mcp.woof.systems\") and (${_ua})"
fi
...
if [[ -n "$WAF_EXPR" ]]; then
  WAF_BODY="$(jq -n --arg expr "$WAF_EXPR" '{
    rules: [{
      action: "block",
      description: "Atlas: bekannte aggressive SEO-/Scraper-Bots blocken (keine AI-Crawler, keine generischen HTTP-Clients)",
      expression: $expr
    }]
  }')"
else
  ...
  rules: [
    {
      ...
      description: "Atlas: API-JSON am Edge cachen, Origin-Cache-Control respektieren",
    ...
    {
      action: "set_cache_settings",
      description: "Atlas: GPT-Actions-Traffic NICHT am Edge cachen (kanal-gebundene Antworten, Cache-Poisoning in beide Richtungen vermeiden)",
